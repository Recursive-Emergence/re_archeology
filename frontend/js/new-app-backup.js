...
class REArchaeologyApp {
    ...
    calculateScanParameters() {
        ...
        // High zoom/small areas likely to get high resolution data (sub-meter) should use airplane
        // Note: This is just initial guess - final icon will be determined by actual backend data resolution
    ...
    startScanningAnimation(iconType = 'satellite', actualResolution = null) {
        ...
        // Initialize animation state with parameters based purely on icon type
        const isHighRes = iconType === 'airplane';
        ...
        this.animationState = {
            ...
            // Scanning parameters based purely on icon type (data resolution)
            cols: isHighRes ? 16 : 12,
            rows: isHighRes ? 12 : 8,
            cycleDuration: isHighRes ? 25000 : 20000,
    ...
    animateScanningIcon(iconElement) {
        ...
        const updatePosition = () => {
            ...
            try {
                ...
                // Clear any continuous beam - only show beams when processing tiles
    ...
    updateAnimationProgress(tileData) {
        if (this.animationState && this.animationState.isActive) {
            ...
            if (tileCenterLat && tileCenterLon) {
                // Set processing flag to prevent clearing beam in animation loop
                this.animationState.isProcessingTile = true;
                
                // Show beam to target tile
                this.drawSatelliteBeam(L.latLng(tileCenterLat, tileCenterLon));
                
                // Add visual feedback - satellite pulses when targeting
                if (this.scanningIcon) {
                    this.scanningIcon.style.transform = 'scale(1.2)';
                    this.scanningIcon.style.filter = 'drop-shadow(0 2px 8px rgba(255, 255, 255, 0.8))';
                }
                
                // Clear beam and reset after processing delay
                setTimeout(() => {
                    this.clearSatelliteBeam();
                    ...
                    if (this.scanningIcon) {
                        ...
                        this.scanningIcon.style.filter = 'drop-shadow(0 2px 4px rgba(0, 0, 0, 0.5))';
                    }
                }, 800); // Show beam for 800ms, then clear
            }
        }
    }
    ...
    handleWebSocketMessage(data) {
        ...
        // Log current session states for debugging
        if (data.session_id) {
            console.log('🔍 Session ID comparison:');
            console.log(`  Message session: ${data.session_id}`);
            console.log(`  Current heatmap session: ${this.currentLidarHeatmapSession}`);
            console.log(`  Current lidar session: ${this.currentLidarSession}`);
        ...
        switch (data.type) {
            ...
            case 'session_completed':
                console.log('✅ Session completed via WebSocket:', data);
                this.handleSessionComplete(data);
                break;
                
            case 'session_failed':
                console.log('❌ Session failed via WebSocket:', data);
                this.handleSessionFailed(data);
                break;
                
            case 'lidar_completed':
                console.log('✅ LiDAR scan completed via WebSocket:', data);
    ...
    handleLidarTileUpdate(data) {
        ...
        // Debug: Log the actual_resolution field specifically
        if (data.actual_resolution) {
            console.log(`📏 Received actual_resolution: "${data.actual_resolution}"`);
        } else {
            console.log('⚠️ No actual_resolution field in tile data');
        }
        
        // Update resolution display and icon based on actual resolution from first tile
        if (data.actual_resolution && !this.animationState?.actualResolution) {
            console.log(`🔄 Processing actual_resolution: ${data.actual_resolution}`);
    ...
    handleLidarHeatmapTileUpdate(data) {
        ...
        // Debug: Log the actual_resolution field specifically
        if (data.tile_data?.actual_resolution) {
            console.log(`📏 Received actual_resolution in heatmap tile: "${data.tile_data.actual_resolution}"`);
        } else {
            console.log('⚠️ No actual_resolution field in heatmap tile data');
        }
        
        // Update resolution display and icon based on actual resolution from first tile
        if (data.tile_data?.actual_resolution && !this.animationState?.actualResolution) {
            console.log(`🔄 Processing heatmap actual_resolution: ${data.tile_data.actual_resolution}`);
            this.updateResolutionDisplay(data.tile_data.actual_resolution);
            this.updateAnimationForResolution(data.tile_data.actual_resolution);
        }
        
        // Update animation progress with tile data
        if (data.tile_data) {
    ...
    /**
     * Handle session failure message from WebSocket
     */
    handleSessionFailed(data) {
        console.log('❌ Session failed via WebSocket:', data);
        
        // Check if this failure is for any of our active sessions
        const isOurHeatmapSession = data.session_id === this.currentLidarHeatmapSession;
        const isOurRegularSession = data.session_id === this.currentLidarSession;
        const isOurCurrentSession = data.session_id === this.currentSession;
        
        if (isOurHeatmapSession || isOurRegularSession || isOurCurrentSession) {
            console.log('🛑 Cleaning up failed session:', data.session_id);
            
            // Use the comprehensive cleanup method
            this.cleanupAfterStop();
            
            // Also clear any remaining session references
            if (isOurHeatmapSession) this.currentLidarHeatmapSession = null;
            if (isOurRegularSession) this.currentLidarSession = null;
            if (isOurCurrentSession) this.currentSession = null;
            
            // Ensure scanning state is reset
            this.isScanning = false;
            
            // Reset scan area to allow new scans
            if (this.scanAreaRectangle) {
                this.scanAreaRectangle.setStyle({ 
                    interactive: true,
                    weight: this.calculateOptimalBorderWeight()
            ...
            if (data.error) {
                console.error(`❌ Session failed with error: ${data.error}`);
                // You could show a toast notification here
            }
            
            console.log(`❌ Session failure handled for session ${data.session_id}`);
        } else {
            console.log(`ℹ️ Received failure for unknown session ${data.session_id}`);
        }
    }

    /**
     * Force satellite mode for data sources that should always use satellite
     */
    forceSatelliteMode(reason) {
        console.log(`🛰️ Forcing satellite mode: ${reason}`);
        
        if (this.animationState) {
            this.animationState.iconType = 'satellite';
            this.animationState.actualResolution = reason;
            this.animationState.smoothing = 0.3;
            
            // Update the actual icon if it exists
            if (this.scanningIcon) {
                this.scanningIcon.innerHTML = '🛰️';
                this.scanningIcon.className = 'scanning-icon satellite';
                this.scanningIcon.style.fontSize = '24px';
                console.log('✅ Switched scanning icon from airplane to satellite');
            }
        }
        
        // Update resolution display
        this.updateResolutionDisplay(reason);
    }
    ...
    calculateOptimalBorderWeight() {
        ...
        // Adaptive weight: more weight at higher zoom for better visibility
        if (zoom >= 16) return 3;
    ...
    updateResolutionDisplay(actualResolution) {
        if (actualResolution && actualResolution !== 'Determining...') {
            this.showResolutionBadge(actualResolution);
    ...
    updateAnimationForResolution(actualResolution) {
        if (this.animationState && actualResolution) {
            ...
            // Detect high resolution data (≤1m) - should use airplane
            const isHighRes = this.isHighResolutionData(actualResolution);
            
            if (isHighRes && this.animationState.iconType !== 'airplane') {
                // Switch to airplane for high resolution data
                console.log(`🎯 Switching to airplane icon for high resolution: ${actualResolution}`);
                
                // Update animation parameters for high resolution
                this.animationState.iconType = 'airplane';
                this.animationState.cols = 16;
                this.animationState.rows = 12;
                this.animationState.cycleDuration = 25000;
                this.animationState.smoothing = 0.2;
                
                // Update the actual icon if it exists
                if (this.scanningIcon) {
                    this.scanningIcon.innerHTML = '✈️';
                    this.scanningIcon.className = 'scanning-icon airplane';
                    this.scanningIcon.style.fontSize = '28px';
                }
            } else if (!isHighRes && this.animationState.iconType !== 'satellite') {
                // Switch to satellite for lower resolution data
                console.log(`🛰️ Using satellite icon for resolution: ${actualResolution}`);
                
                // Update animation parameters for standard resolution
                this.animationState.iconType = 'satellite';
                this.animationState.cols = 12;
                this.animationState.rows = 8;
                this.animationState.cycleDuration = 20000;
                this.animationState.smoothing = 0.3;
            ...
            console.log(`🎯 Updated animation for resolution: ${actualResolution} (${isHighRes ? 'airplane' : 'satellite'})`);
        }
    }
    
    /**
     * Determine if resolution data is high resolution (sub-meter only)
     * Only sub-meter data should use airplane - 1m and above should use satellite
     */
    isHighResolutionData(actualResolution) {
        ...
        const resolutionStr = actualResolution.toLowerCase();
        
        console.log(`🔍 Analyzing resolution string: "${actualResolution}" -> "${resolutionStr}"`);
        
        // IMPORTANT: Check for pixel resolution indicators first (these are NOT data resolution)
        if (resolutionStr.includes('/px') || resolutionStr.includes('pixel') || 
            resolutionStr.includes('display') || resolutionStr.includes('target_res') ||
            resolutionStr.includes('m/px')) {
            console.log(`⚠️ Detected pixel/display resolution, not data resolution: ${actualResolution}`);
            return false; // Always use satellite for pixel resolution specs
        }
        
        // Parse numeric values if format is "X.Xm" or "Xm"
        const match = resolutionStr.match(/(\d+\.?\d*)\s*m(?![/px])/); // Don't match "m/px"
        if (match) {
            const resolution = parseFloat(match[1]);
            
            // CRITICAL: Values like "0.5m" in the context of SRTM data are often pixel resolution
            // If the resolution seems suspiciously low (like 0.5m) but we know this is from SRTM,
            // it's likely pixel resolution, not data resolution. SRTM data is 30m resolution.
            if (resolution <= 1.0) {
                console.log(`🧐 Sub-meter value detected (${resolution}m) - checking if this is actually pixel resolution...`);
                
                // For now, be conservative: values like 0.5m without clear "data resolution" context
                // are likely pixel resolution from low-res data sources like SRTM
                console.log(`🛰️ Treating ${resolution}m as likely pixel resolution (SRTM context) -> satellite mode`);
                return false; // Use satellite for suspected pixel resolution
            }
            
            const isHighRes = resolution < 1.0;
            console.log(`📏 Parsed resolution: ${resolution}m -> ${isHighRes ? 'airplane' : 'satellite'}`);
            return isHighRes; // Only confirmed sub-meter data resolution is airplane-worthy
        }
        
        // Check for explicit sub-meter data resolutions (airplane-worthy)
        if (resolutionStr.includes('25cm') || resolutionStr.includes('50cm') || 
            resolutionStr.includes('10cm') || resolutionStr.includes('20cm') ||
            resolutionStr.includes('lidar') || resolutionStr.includes('ahn') ||
            resolutionStr.includes('high') && (resolutionStr.includes('0.') || resolutionStr.includes('cm'))) {
            console.log(`✈️ High resolution data source detected: ${actualResolution}`);
            return true;
        }
        
        // Explicitly exclude 1m and above (should use satellite)
        if (resolutionStr.includes('1m') || resolutionStr.includes('1.0m') || 
            resolutionStr.includes('1 m') || resolutionStr.includes('1.0 m') ||
            resolutionStr.includes('2m') || resolutionStr.includes('5m') ||
            resolutionStr.includes('10m') || resolutionStr.includes('30m') ||
            resolutionStr.includes('15m') || resolutionStr.includes('20m') ||
            resolutionStr.includes('60m') || resolutionStr.includes('90m')) {
            console.log(`🛰️ Standard/low resolution detected (≥1m): ${actualResolution}`);
            return false;
        }
        
        console.log(`❓ Could not determine resolution type for: ${actualResolution}`);
        return false; // Default to satellite for unknown resolutions
    }
    
    /**
     * Handle LiDAR scan completion message from WebSocket
     */
    handleLidarCompleted(data) {
        console.log('✅ LiDAR scan completed via WebSocket:', data);
        
        // Check if this completion is for any of our active sessions
        const isOurHeatmapSession = data.session_id === this.currentLidarHeatmapSession;
        const isOurRegularSession = data.session_id === this.currentLidarSession;
        const isOurCurrentSession = data.session_id === this.currentSession;
        
        if (isOurHeatmapSession || isOurRegularSession || isOurCurrentSession) {
            console.log('🛑 Cleaning up completed session:', data.session_id);
            ...
            console.log(`✅ LiDAR scan completion handled for session ${data.session_id}`);
        } else {
            console.log(`ℹ️ Received completion for unknown session ${data.session_id}`);
        }
    }
